...
export async function createAdminSettings(req, res) {
  ...
  try {
    const adminSettingsId = await AdminSettingsModel.createAdminSettings(
      appVersion,
      maintenanceActive,
    );
    res.status(201).json({
...
export async function getAdminSettings(req, res) {
  ...
  try {
    const adminSettings =
      await AdminSettingsModel.getAdminSettingsById(adminSettingsId);
    res.status(200).json({
      status: 200,
...
export async function updateAdminSettings(req, res) {
  ...
  try {
    await AdminSettingsModel.updateAdminSettingsById(
      adminSettingsId,
      newAppVersion,
